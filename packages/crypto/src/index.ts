 ...
 *  - `@largerio/crypto`        — this barrel: client + server primitives (backend).
 *  - `@largerio/crypto/client` — client-only, plus the MessagePack decode helpers
 *                              (`decodePayloadBytes`/`decodeRawBytes`); excludes
 *                              server code so the browser bundle stays lean.
 *  - `@largerio/crypto/server` — server-only (AES-256-GCM) primitives.
 */
