...
- **`packages/sdk-js`** — JS/TS SDK for Secret instances
  ...
  - Re-exports types from `@largerio/shared`
...
- **SDK-first frontend**: The web app consumes `@largerio/sdk-js` (dog-fooding). No direct API calls or crypto operations in the frontend.
...
- Crypto tests require `await initSodium()` in `beforeAll`; cross-language vectors are generated by `generate-test-vectors.test.ts` into `packages/shared/src/test-vectors/vectors.json`
